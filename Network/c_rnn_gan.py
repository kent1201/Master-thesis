...
class Generator(nn.Module):
    ''' C-RNN-GAN generator
    '''
    def __init__(self, num_feats, cuda_device, hidden_units=256, drop_prob=0.6):
    ...
    def forward(self, z, states):
        ...
        if self.cuda_device:
            prev_gen = prev_gen.to(self.cuda_device)
    ...
    def init_hidden(self, batch_size):
        ...
        if (self.cuda_device):
            hidden = ((weight.new(batch_size, self.hidden_dim).zero_().to(self.cuda_device),
                       weight.new(batch_size, self.hidden_dim).zero_().to(self.cuda_device)),
                      (weight.new(batch_size, self.hidden_dim).zero_().to(self.cuda_device),
                       weight.new(batch_size, self.hidden_dim).zero_().to(self.cuda_device)))
        else:
    ...
    def init_hidden(self, batch_size):
        ...
        if self.cuda_device:
            hidden = (weight.new(self.num_layers * layer_mult, batch_size,
                                 self.hidden_dim).zero_().to(self.cuda_device),
