...
class Discriminator(nn.Module):
  def __init__(self, module='gru', time_stamp=82, input_size=24, hidden_dim=17, output_dim=1, num_layers=10, activate_function=None):
  ...
  def forward(self, X, H):
    if self.module == 'tcn':
      ...
      X = self.Tanh(self.conv2(self.conv1(X)))
    ...
    if(self.activate != None):
      Y = self.fc(self.activate(output))
    else:
      Y = self.fc(output)
    return Y
