...
synthetic_dataset_dir = config.get('GenTstVis', 'syntheticDataset_path') + '/' + config.get(
    'GenTstVis', 'date_dir') + '/' + config.get('GenTstVis', 'classification_dir')
...
if __name__ == '__main__':
    ...
    real_dataset = SensorSignalDataset(
        root_dir=real_dataset_dir, transform=False)
    synthetic_dataset = SensorSignalDataset(
        root_dir=synthetic_dataset_dir, transform=False)

    real_data_loader = DataLoader(
        dataset=real_dataset, batch_size=144, shuffle=True, num_workers=1)
    synthetic_data_loader = DataLoader(
        dataset=synthetic_dataset, batch_size=144, shuffle=True, num_workers=1)

    real_data_loader_iter = iter(real_data_loader)
    synthetic_data_loader_iter = iter(synthetic_data_loader)

    real_data, _ = next(real_data_loader_iter)
    synthetic_data, _ = next(synthetic_data_loader_iter)

    real_data_list = []
    synthetic_data_list = []

    for i in range(0, len(real_data)):
        real_data_list.append(real_data[i].numpy())
        synthetic_data_list.append(synthetic_data[i].numpy())


    if not os.path.exists(pic_path):
        os.makedirs(pic_path)

    visualization(real_data_list, synthetic_data_list, 'pca', pic_path, pca_pic_name)
    visualization(real_data_list, synthetic_data_list, 'tsne', pic_path, t_sne_pic_name)
